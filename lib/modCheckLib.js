...
class modFileCollection {
	...
	getCurVerCollections(ver) {
		...
		return folderData.sort((a, b) => new Intl.Collator().compare(a[1], b[1]))
	}
	...
	#doAlphaSort() {
		for ( const collectKey of this.#set_Collections ) {
			this.#list_allMods[collectKey].alphaSort.sort(new Intl.Collator().compare)
		}
	}
...
class savegameTrack {
	...
	async getInfo() {
		try {
			...
			this.#saveMods.byDate.sort(new Intl.Collator().compare)
...
class badgeHandle {
	 ...
	 */
	get keyList() {
		const sorted      = [...this.#baseSet].sort((a, b) => new Intl.Collator().compare(this.#l10nNames[a], this.#l10nNames[b]))
		const returnArray = []
		...
		for ( const key of sorted ) {
			returnArray.push({
				...
				title : [...this.#extraData[key].title].sort(new Intl.Collator().compare).join(', '),
			})
		}
		return returnArray
	}
