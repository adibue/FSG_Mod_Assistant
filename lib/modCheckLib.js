...
class modFileCollection {
	...
	#startThreads() {
		...
		for ( let idx = 1; idx <= this.#threadPoolMax; idx++ ) {
			...
			thisThread.on('exit',    () => {
				if ( this.#threadPoolIdx.has(idx-1) ) {
					this.#log.danger(`worker-thread-${idx}`, 'Worker exited before confirming its queue was flushed')
				}
			})
		...
		if ( Object.hasOwn(m, 'type') ) {
			switch (m.type) {
				...
				case 'modRecord' :
					...
					break
				case 'workerDone' :
					this.#workQueueDone(m.workerID)
					this.#threadPool[m.workerID]?.send({type : 'workerDoneAck'})
					break
				default :
					break
			}
	...
	#workQueueDone(idx) {
		if ( !this.#threadPoolIdx.has(idx) ) { return }
	...
	async processMods() {
		...
		await Promise.all(this.#scanPromise)
		this.#exitThreads()
	}
...
class saveFileChecker {
	 ...
	 */
	async getInfo() {
		this.#fileHandle = new fileHandlerAsync(this.#fileName, this.#isFolder, this.#log, this.#silent)
...
class modPackChecker {
	...
	async getInfo() {
		...
		return [true, this.#fileHandle.list]
	}
}
...
class inputBinds {
	...
	constructor(filePath) {
		...
		this.#xmlParser = fileHandlerAsync.getParser('inputBind')
	}
