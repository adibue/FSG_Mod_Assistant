...
const { parseModLastPass, parseSaveGame } = require('fs_mod_parser_neon')

function normalizeModHubName(name) {
	return typeof name === 'string' ? name.trim().replace(/\.zip$/iu, '').toLocaleLowerCase() : ''
}

function normalizeModHubLooseName(name) {
	return normalizeModHubName(name)
		.replace(/^fs(?:19|22|25)[_-]?/u, '')
		.replace(/[^a-z0-9]/gu, '')
}
...
class modFileCollection {
	...
	#modHubVersion = {}
	#modHubNameIndex = new Map()
	#modHubLooseNameIndex = new Map()
	...
	#threadPoolCount = 0
	#threadPoolMax   = require('node:os').cpus().length
	#threadCurrent   = 0
	#threadDoneEmit  = null
				...
				request.on('response', (response) => {
					...
					response.on('end',  () => {
						try {
							...
							this.#log.info('fsg-bot-poll', 'File error', err.message)
						}
	...
	mapColAndUUIDToFullPath(key, uuid) {
		const folder = this.#map_CollectionToFolder[key]
		const mod    = this.#list_allMods?.[key]?.mods?.[uuid] || null

		if ( mod === null || typeof folder === 'undefined' ) { return null }
		return path.join(folder, path.basename(mod.fileDetail.fullPath))
	}
	...
	set modHubList(newList)    {
		this.#modHubList = newList
		this.#modHubNameIndex = new Map()
		this.#modHubLooseNameIndex = new Map()
		for ( const [modName, modHubID] of Object.entries(newList?.mods ?? {}) ) {
			const normalizedName = normalizeModHubName(modName)
			const looseName = normalizeModHubLooseName(modName)
			if ( normalizedName === '' ) { continue }
			if ( this.#modHubNameIndex.has(normalizedName) ) {
				this.#modHubNameIndex.set(normalizedName, null)
			} else {
				this.#modHubNameIndex.set(normalizedName, modHubID)
			}
			if ( looseName === '' ) { continue }
			if ( this.#modHubLooseNameIndex.has(looseName) ) {
				this.#modHubLooseNameIndex.set(looseName, null)
			} else {
				this.#modHubLooseNameIndex.set(looseName, modHubID)
			}
		}
	}
	...
	modHubFullRecord(thisMod) {
		const match = this.modHubMatchShortName(thisMod.fileDetail.shortName)
		const modHubID = match.id

		return {
			confidence : match.confidence,
			id      : modHubID,
			match   : match.method,
			recent  : modHubID !== null && this.#modHubList.last.includes(parseInt(modHubID)),
			version : this.modHubVersionModHubId(modHubID),
		}
	}

	modHubMatchShortName(shortName) {
		if ( typeof shortName !== 'string' || shortName === '' ) {
			return { confidence : 'none', id : null, method : 'unmatched' }
		}
		if ( Object.hasOwn(this.#modHubList.mods, shortName) ) {
			return { confidence : 'exact', id : this.#modHubList.mods[shortName], method : 'filename' }
		}

		const normalizedName = normalizeModHubName(shortName)
		const normalizedID = this.#modHubNameIndex.get(normalizedName)
		if ( typeof normalizedID !== 'undefined' && normalizedID !== null ) {
			return { confidence : 'high', id : normalizedID, method : 'filename-case-insensitive' }
		}

		const savedRecords = serveIPC.storeLibrary?.get?.('records', {}) ?? {}
		const savedIDs = new Set(Object.values(savedRecords)
			.filter((record) => [record?.fileName, ...(record?.modNames ?? [])].some((name) => normalizeModHubName(name) === normalizedName))
			.flatMap((record) => record?.modHubIDs ?? []))
		if ( savedIDs.size === 1 ) {
			return { confidence : 'high', id : [...savedIDs][0], method : 'saved-modhub-id' }
		}

		const looseID = this.#modHubLooseNameIndex.get(normalizeModHubLooseName(shortName))
		if ( typeof looseID !== 'undefined' && looseID !== null ) {
			return { confidence : 'medium', id : looseID, method : 'normalized-filename' }
		}
		return { confidence : 'none', id : null, method : 'unmatched' }
	}
	...
	modHubModRecord(thisMod) {
		return this.modHubMatchShortName(thisMod.fileDetail.shortName).id
	}
	...
	modHubModUUID(modUUID) {
		return this.modHubMatchShortName(this.#map_ModUUIDToShortName[modUUID]).id
	}
	...
	modVerListFiltered(modShortName) {
		const modSet    = []
		const theseMods = this.#map_ShortName[modShortName]

		for ( const [thisCollect, thisModUUID] of theseMods ) {
			const isFrozen = this.#map_CollectionNotes.get(`${thisCollect}.notes_frozen`, false)
			const thisMod  = this.#list_allMods[thisCollect].mods[thisModUUID]

			if ( thisMod.fileDetail.isFolder || isFrozen ) { continue }

			modSet.push({
				collectKey  : thisCollect,
				collectName : this.#map_CollectionToName[thisCollect],
				modRecord   : thisMod,
				version     : thisMod.modDesc.version,
			})
		}
		return modSet
	}
	...
	getCurCollectSafe(folder) {
		if ( folder === null || typeof folder !== 'string' ) { return '' }
		return this.mapCollectionToName(this.mapFolderToCollection(folder))
	}

	getCurVerCollections(ver) {
		const folderData = []
		for ( const [CKey, status] of Object.entries(this.#map_CollectionToStatus) ) {
			if ( status && this.version(CKey) === ver ) {
				if ( typeof this.mapCollectionToName(CKey) === 'undefined' ) { continue }
				folderData.push([
					CKey,
					this.mapCollectionToName(CKey),
					this.mapCollectionToFullName(CKey),
					this.getSafeDate(CKey, 'add_date'),
					this.getSafeDate(CKey, 'last')
				])
			}
		}
		return folderData.sort((a, b) => Intl.Collator().compare(a[1], b[1]))
	}
	...
	#doModBadge(modRecord, hasConflicts = false, hasDepend = false, requireSet) {
		...
		if ( thisMHRec.id !== null ) {
			if ( compareModVersions(modRecord.modDesc.version, thisMHRec.version) < 0 ) {
				newBadge.add('update')
			}
			if ( thisMHRec.recent ) {
				newBadge.add('recent')
	...
	#cacheHit(thisFileStats) {
		if ( thisFileStats.folder || this.#skipCache || thisFileStats.hashString === null ) {
			return false
		}
	...
	#fileStats(collectKey, thisFile) {
		...
		try {
			...
			if ( fileStats.isFolder ) {
				...
				fileStats.hashString = this.#getMD5Hash(`${thisFile.name}-${fileStats.size}-${fileStats.birthDate.toISOString()}`)
			}
		} catch (err) {
	...
	#util_gameVersion(dVer) {
		...
		if ( this.#util_between(60, dVer, 85) ) { return 22 }
		if ( dVer >= 90 ) { return 25 }
		return 0
	}
	...
	#addModToData(collectKey, modRecord) {
		...
		thisModRecord.has_brands        = thisModRecord.includeDetail?.itemBrands || []
		thisModRecord.has_cats          = thisModRecord.includeDetail?.itemCategories || []
		
		if ( thisModRecord.issues.includes('MALICIOUS_CODE') || thisModRecord.issues.includes('MALICIOUS_FILE') ) {
		...
		this.#list_allDetail[`${collectKey}--${thisModRecord.uuid}`] = thisModRecord.includeDetail
		thisModRecord.includeDetail = modRecord.detailIconLoaded ? 2 : 1
	...
	async getAndUpdateDetail(fullID) {
		return Promise.all(this.#scanPromise).then(() => {
			const idParts = fullID.split('--')
			const currentRecord = this.#list_allMods?.[idParts[0]]?.mods?.[idParts[1]]

			this.#log.debug('modlook', `Updated modRecord :: ${fullID}`)
			const newRecord = JSON.parse(parseModLastPass(currentRecord.fileDetail.fullPath))
			this.#list_allDetail[fullID] = newRecord.includeDetail
			currentRecord.includeDetail = 2

			return this.detailMod(fullID)
		})
	}
	 ...
	 */
	version(collectKey) {
		return this.#map_CollectionNotes.get(`${collectKey}.notes_version`, 25)
	}
	isHolding(collectKey) {
		return this.#map_CollectionNotes.get(`${collectKey}.notes_holding`, false)
	}
	...
	getSafeDate(collectKey, noteKey) {
		const safeDate = this.getSafeNote(collectKey, noteKey)
		return typeof safeDate === 'object' ? safeDate.toISOString() : safeDate
	}
	...
	async detailMod(fullID) {
		return Promise.all(this.#scanPromise).then(() => {
			const idParts = fullID.split('--')
			return [
				this.#list_allMods?.[idParts[0]]?.mods?.[idParts[1]] || null,
				this.#list_allDetail[fullID]
			]
		})
	}
	async renderMod(fullID) {
		return Promise.all(this.#scanPromise).then(() => {
			const idParts = fullID.split('--')
			return this.#list_allMods?.[idParts[0]]?.mods?.[idParts[1]] || null
		})
	}
	async renderBindConflict() {
		return Promise.all(this.#scanPromise).then(() => {
			return this.#bindConflict
		})
	}
	async renderCollectNotes(key) {
		return Promise.all(this.#scanPromise).then(() => this.#doNotesDefault()[key] )
	}

	isValidForList(CKey, currentVer) {
		if ( this.version(CKey) === currentVer ) { return true }
		return this.isHolding(CKey)
	}

	get allModList() {
		const returnList = {}
		const currentVer = this.#settings.get('game_version')

		for ( const CKey of this.collections ) {
			returnList[CKey] = this.#list_allMods[CKey]
			if ( !this.isValidForList(CKey, currentVer) ) {
				returnList[CKey].mods = {}
			}
		}
		return returnList
	}
	...
	async toRenderer(extra = null) {
		return Promise.all(this.#scanPromise).then(() => {
			return {
				...
				modList                    : this.allModList, //this.#list_allMods,
				opts                       : extra,
...
class savegameTrack {
	...
	async getInfo() {
		try {
			...
			const currentSave    = JSON.parse(parseSaveGame(this.#savePath))

			if ( !currentSave.isValid ) { throw new Error('Original save could not be read') }

			this.#saveMods.current = Object.keys(currentSave.mods)
			this.#saveMods.saveID  = path.basename(this.#savePath)

			const currentModList = Object.keys(currentSave.mods)

			for ( const thisBackup of backupFileList ) {
				const theseMods    = JSON.parse(parseSaveGame(thisBackup))
				// console.log(theseMods)
				if ( !theseMods.isValid  || theseMods.map !== currentSave.map ) { continue }


				const thisModList = Object.keys(theseMods.mods)

				const onlyBackup   = thisModList.filter((x) => !currentModList.includes(x))
				const onlyOriginal = currentModList.filter((x) => !thisModList.includes(x))
...
class saveGameManager {
		...
		return fsPromise.readdir(this.#fullPath, {withFileTypes : true}).then(async (folderContents) => {
			for ( const thisFolder of folderContents ) {
				if ( !thisFolder.isDirectory() ) {
					continue
				} else if ( thisFolder.name.match(/^savegame\d{1,2}$/) ) {
	...
	async #findBackupSaves() {
		...
		return fsPromise.readdir(path.join(this.#backPath), { withFileTypes : true }).then(async (folderContents) => {
			for ( const thisFolder of folderContents ) {
				if ( !thisFolder.isDirectory() || !thisFolder.name.match(/^savegame\d{1,2}_back/) ) {
					continue
				} else {
					try {
						...
						this.#allSaves[thisID].backups.push({
							...
							uuid     : crypto.createHash('md5').update(path.join(this.#backPath, thisFolder.name)).digest('hex'),
...
class badgeHandle {
	...
	get keyList() {
		...
		for ( const key of sorted ) {
			returnArray.push({
				name : key,
				class : [...this.#extraData[key].cls],
				title : [...this.#extraData[key].title].sort(Intl.Collator().compare).join(', '),
			})
		}
		return returnArray
	}
...
class inputBinds {
	...
	constructor(filePath) {
		...
		this.#log      = serveIPC.log.group('input_bind_manager')

		const alwaysArray = new Set([
			'inputbinding.actionbinding',
			'inputbinding.actionbinding.binding',
			'inputbinding.devices.device',
		])
		this.#xmlParser = XMLParser({
			...
			ignorePiTags           : true,
			isArray                : (_, jPath) => alwaysArray.has(jPath),
	...
	async getInfo() {
		try {
			...
			const thisXML = this.#xmlParser.parse(fs.readFileSync(this.#filePath, 'utf-8'))?.inputbinding ?? null
			
			if ( thisXML === null ) {
				throw new Error(`Parsing file failed: ${this.#filePath}`)
			}
