...
const baseGameLang = {
	25 : require('./modLookerLang_25.json'),
	22 : require('./modLookerLang_22.json'),
	19 : require('./modLookerLang_19.json'),
}
/**
 * Logger Class
 * @class
...
class translator {
	...
	async getTextOverride(key, version = 22, {newText = null, prefix = '', suffix = ''} = {}) {
		return this.getText(key, version, true).then((result) => {
	...
	async getText(key, version = 22, skipDebug = false) {
		...
		return this.#langPromise.then(() => {
			...
			if ( i18nObject.entry_key.startsWith('$l10n') ) {
				...
				i18nObject.entry ??= baseGameLang?.[version]?.[this.#currentLocale]?.[baseKey]
				i18nObject.entry ??= baseGameLang?.[version].en?.[baseKey]
...
class modCacheManager {
	...
	constructor(filePath) {
		...
		this.#saveFileDetail = path.join(filePath, 'mod_items.json')
		this.loadFile()
	}
	 ...
	 */
	loadFile() {
		if ( !fs.existsSync(this.#saveFileData) || !fs.existsSync(this.#saveFileIcon) || !fs.existsSync(this.#saveFileDetail) ) {
		...
		try {
			...
			const itemContents = JSON.parse(fs.readFileSync(this.#saveFileDetail))

			for ( const key of Object.keys(dataContents) ) {
				const keyContents = dataContents[key]

				keyContents.modDesc.iconImage = iconContents[key] ?? null
				keyContents.includeDetail = itemContents[key] ?? null
	...
	saveFile() {
		try {
			...
			const writeItem = fs.createWriteStream(this.#saveFileDetail)
			...
			for ( const [key, value] of this.#cacheMap ) {
				...
				if ( lastItem !== null ) { writeItem.write(`${lastItem},\n`) }

				lastIcon = `"${key}" : ${JSON.stringify(value.modDesc.iconImage)}`
				lastItem = `"${key}" : ${JSON.stringify(value.includeDetail)}`
				lastData = `"${key}" : ${JSON.stringify(value, (k, v) => k === 'iconImage' || k === 'includeDetail' ? null : v )}`
			}
