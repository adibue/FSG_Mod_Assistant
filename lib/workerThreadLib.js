...
class ddsDecoder {
	...
	async parseDDS(fileBuffer, hasAlpha = false, mapCrop = false) {
		...
		try {
			...
			throw new Error(`DDS Header Error: ${err}`, { cause : err })
		}
		...
		try {
			...
			throw new Error(`DDS DXT10 Conversion Error: ${err}`, { cause : err })
		}
		...
		try {
			...
			return await vipsPromise.then((vReady) => {
				...
				throw new Error(`WEBP Encode Error: ${err}`, { cause : err })
			})
		} catch (err) {
			throw new Error(`DDS Decode Error: ${err}`, { cause : err })
		}
	}
}
