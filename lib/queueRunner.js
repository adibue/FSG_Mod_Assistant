...
process.on('message', (message) => {
	if ( message?.type === 'workerDoneAck' && workerDoneResolve !== null ) {
		workerDoneResolve()
		...
		switch (workType) {
			...
			case 'exit' :
				if ( workQueue.length === 0 ) {
					...
					const workerDoneAck = new Promise((resolve) => { workerDoneResolve = resolve })
					await sendToParent({
						type     : 'workerDone',
						workerID : Number(threadNum) - 1,
					})
					await workerDoneAck
					process.exit()
				} else {
...
function sendToParent(m) {
	return new Promise((resolve) => {
		...
		process.send(
			{...m, pid : `${threadNum}::${process.pid.toString().padStart(5, '0')}`},
			() => { resolve() }
		)
	})
}
