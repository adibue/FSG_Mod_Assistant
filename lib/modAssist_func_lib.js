...
const general = {
	...
	doBootLog : () => {
		...
		thisLog.info(`FS Mod Assistant Community : v${app.getVersion()}`)
...
const wizard = {
	initMain    : () => {
		const theseFinds = wizard.getSettings(false, true)
		const versions   = [25, 22, 19, 17, 15, 13]

		for ( const version of versions ) {
			serveIPC.path.games[version]    = theseFinds.games[version]?.[0]?.[1] ?? ''
			serveIPC.path.settings[version] = theseFinds.settings[version]?.[0] ?? ''
		}

		const detectedVersion = versions.find((version) => (
			serveIPC.path.games[version] !== '' || serveIPC.path.settings[version] !== ''
		)) ?? 22

		serveIPC.path.gameVersion = detectedVersion
		serveIPC.path.game        = serveIPC.path.games[detectedVersion]
		serveIPC.path.setFile     = serveIPC.path.settings[detectedVersion]

		if ( serveIPC.path.setFile !== '' ) {
			serveIPC.path.setFolder = path.dirname(serveIPC.path.setFile)
		} else {
	...
	getSettings_game : (only22 = false) => {
		...
		const allGameGuesses = {
			...
			25 : [
				...
				['XBox',  'C:\\XboxGames\\Farming Simulator 25 - Window 10 Edition\\Content\\gamelaunchhelper.exe'],
			],
		}
	...
	getSettings_mods : (settingsPaths) => {
		...
		for ( const modFolder of settingsPaths ) {
			...
			if ( fs.existsSync(thisModFolder) ) {
				...
				for ( const thisEntry of folderContents ) {
					if ( thisEntry.isDirectory() && ! thisEntry.name.startsWith('FS')) {
						foundCollections.add(path.join(thisModFolder, thisEntry.name))
					}
			...
			try {
				const XMLParser = new fxml.XMLParser({ ignoreAttributes : false })
				const XMLDoc    = XMLParser.parse(fs.readFileSync(modFolder, 'utf8'))
				const override  = XMLDoc?.gameSettings?.modsDirectoryOverride
				const isActive  = override?.['@_active'] === true || override?.['@_active'] === 'true'
				const directory = override?.['@_directory']

				if ( isActive && typeof directory === 'string' && fs.existsSync(directory) ) {
					foundCollections.add(path.normalize(directory))
				}
			} catch {
				// The main game-settings reader reports malformed files elsewhere.
			}
		}
		returnObj.hasCollections = [...foundCollections]
		return returnObj
	},
