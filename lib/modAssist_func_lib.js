...
// const { inputBinds, saveFileChecker, saveGameManager } = require('./modCheckLib.js')
const { inputBinds, saveGameManager } = require('./modCheckLib.js')
const { parseSaveGame } = require('fs_mod_parser_neon')
...
const runtimeRoot = fs.existsSync(path.join(sourceRoot, 'modAssist_main.js')) ? sourceRoot : packagedRoot
const externalRoot = fs.existsSync(path.join(sourceRoot, 'icon')) ? sourceRoot : path.join(process.resourcesPath, '..')

// MARK: GENERAL
const general = {
	getPackPathAsar   : (...parts) => path.join(runtimeRoot, ...parts),
	getPackPathRender : (...parts) => general.getPackPathAsar('renderer', ...parts),
	getPackPathRoot   : (...parts) => {
		const testPath = path.join(externalRoot, ...parts)
	...
	doModCacheCheck : () => {
		...
		if ( serveIPC.storeSet.get('cache_version') !== app.getVersion() ) {
			thisLog.notice('Version Changed, Mod Cache Cleared')
			serveIPC.storeCache.clearAll()
		}

		serveIPC.storeSet.set('wins.mini.h', 50)
	...
	pollGame : async () => {
		if ( ! serveIPC.isGamePolling ) { return }

		const gameExe = `FarmingSimulator20${prefs.ver()}Game.exe`
	
		return require('node:child_process').exec(`tasklist /fi "IMAGENAME eq ${gameExe}" /fo csv /nh`, (err, stdout) => {
			if ( err ) {
	...
	exportCSV : (collection) => {
		...
		for ( const mod of serveIPC.modCollect.getModListFromCollection(collection) ) {
			...
			csvTable.push(general.doCSVRow([
				...
				mod.l10n?.title?.en || mod.l10n?.title?.de || '--',
				mod.modDesc.version,
				mod.modDesc.author,
				modHubYesNo,
...
const discord = {
	...
	setActivity : async () => {
		...
		discord.thisRPC.setActivity({
			details        : custom_detail !== '' ? custom_detail : `${discord_verb} Collection: \n${serveIPC.gameSetOverride.folder !== null && typeof serveIPC.gameSetOverride.folder !== 'undefined' ? path.basename(serveIPC.gameSetOverride.folder) : '--'}`,
			instance       : true,
...
const wizard = {
	initMain    : () => {
		...
		if ( theseFinds.games[25].length !== 0 ) {
			serveIPC.path.game = theseFinds.games[25][0][1]
		}
		if ( theseFinds.settings[25].length !== 0 ) {
			serveIPC.path.setFile = theseFinds.settings[25][0]
		}
		serveIPC.path.setFolder = theseFinds.settings.base
	},

	getSettings : (only25 = false, noMods = false) => {
		const settings     = wizard.getSettings_settings()
		const games        = wizard.getSettings_game(only25)

		return {
			games        : games,
			mods         : !noMods ? [25, 22, 19, 17, 15, 13].map((x) => ({ ver : x, ...wizard.getSettings_mods(settings[x])})) : null,
			settings     : settings,
		}
	},
	getSettings_game : (only25 = false) => {
		let steamPath2VDF
		let steamFolders = []
		try {
		...
		const allGameGuesses = {
			...
			25 : [
				['eShop', 'C:\\Program Files (x86)\\Farming Simulator 2025\\FarmingSimulator2025.exe'],
				['Steam', 'steamapps\\common\\Farming Simulator 25\\FarmingSimulator2025.exe'],
				['Epic',  'C:\\Program Files\\Epic Games\\FarmingSimulator25\\FarmingSimulator2025.exe'],
				['XBox',  'C:\\XboxGames\\Farming Simulator 25 (PC)\\Content\\gamelaunchhelper.exe'],
			],
		}

		const foundGames = {}
		for ( const versionKey of only25 ? [25] : Object.keys(allGameGuesses) ) {
		...
		for ( const modFolder of settingsPaths ) {
			...
			if ( fs.existsSync(thisModFolder) ) {
				...
				for ( const thisEntry of folderContents ) {
					if ( thisEntry.isDirectory() && ! thisEntry.name.startsWith('FS')) {
...
const menu = {
	...
	page_find : ( thisMod ) => [
		...
		...thisMod.collect.map((x) => menu.icon(
			`${x.name} :: ${x.version}`,
			() => {
				serveIPC.windowLib.sendAndFocusValid('main', 'select:withText', x.fullId, thisMod.name)
			},
			'sendCheck'
		))
	],
			...
			click : () => {
				switch ( thisMenuItem[0] ) {
					case 2:
						...
						serveIPC.windowLib.sendToValidWindow('main', 'select:withText', thisMenuItem[2], x)
						break
					case 1:
						// send to find window
						serveIPC.windowLib.raiseOrOpen('find', () => {
							serveIPC.windowLib.sendToValidWindow('find', 'find:filterText', x)
						})
						break
					default:
...
// MARK: MODHUB
const modHub = {
	file_22 : 'modHubDataCombo.json',
	file_25 : 'modHubDataCombo25.json',
	isInit  : false,
	log     : null,
	path_22 : null,
	path_25 : null,
	...
	init : () => {
		if ( ! modHub.isInit ) {
			...
			modHub.isInit  = true
			modHub.path_22 = path.join(app.getPath('userData'), modHub.file_22)
			modHub.path_25 = path.join(app.getPath('userData'), modHub.file_25)
		}
	},
	load : () => {
		modHub.init()
		try {
			...
			let jsonData22 = { mods : {}, recent : [], version : {} }
			let jsonData25 = { mods : {}, recent : [], version : {} }

			if ( fs.existsSync(modHub.path_22 ) ) {
				const rawData22  = fs.readFileSync(modHub.path_22)
				jsonData22 = JSON.parse(rawData22)
			}
			if ( fs.existsSync(modHub.path_25 ) ) {
				const rawData25  = fs.readFileSync(modHub.path_25)
				jsonData25 = JSON.parse(rawData25)
			}

			serveIPC.modCollect.modHubList = {
				mods : { ...oldModHub.mods, ...jsonData22.mods, ...jsonData25.mods},
				last : [...jsonData22.recent, ...jsonData25.recent],
			}
			serveIPC.modCollect.modHubVersion = { ...oldModHub.versions, ...jsonData22.version, ...jsonData25.version}

			modHub.log.debug('Loaded', modHub.file_22, modHub.file_25)
		} catch (err) {
			modHub.log.warning('Loading failed', modHub.file_22, modHub.file_25, err)
		}
	},
	refresh : () => {
		modHub.init()
		if ( net.isOnline() ) {
			const request_22 = net.request('https://jtsage.dev/modHubData22_combo.json')

			request_22.setHeader('pragma', 'no-cache')

			request_22.on('response', (response) => {
				modHub.log.info('Got', modHub.file_22, response.statusCode)
				let responseData = ''
				...
				response.on('end',  () => {
					fs.writeFileSync(modHub.path_22, responseData)
					modHub.load()
				})
			})
			request_22.on('abort', () => {
			...
			request_22.on('error', (err) => {
				modHub.load()
				modHub.log.info('Network error', err)
			})
			request_22.end()

			const request_25 = net.request('https://jtsage.dev/modHubData25_combo.json')

			request_25.setHeader('pragma', 'no-cache')

			request_25.on('response', (response) => {
				modHub.log.info('Got', modHub.file_25, response.statusCode)
				let responseData = ''
				...
				response.on('data', (chunk) => { responseData = responseData + chunk.toString() })
				response.on('end',  () => {
					fs.writeFileSync(modHub.path_25, responseData)
					modHub.load()
				})
			})
			request_25.on('abort', () => {
...
const processor = {
	...
	addFolderTracking : (folder, overrideVersion = null, json_import = false) => {
		const newVersion = overrideVersion !== null ? overrideVersion : prefs.ver()
		...
		general.toggleFolderDirty()
		prefs.saveFolders()
		serveIPC.storeNote.set(`${collectKey}.notes_version`, newVersion)
...
const prefs = {
	...
	basePath : () => path.dirname(prefs.verGet('game_settings', prefs.ver())),
	unVerKey : (key) => parseInt(key.split('_')[2] || 22),
	ver      : () => serveIPC.storeSet.get('game_version'),
	verGet   : (key, version = 22 ) => serveIPC.storeSet.get(prefs.verKey(key, version)),
	verKey   : (key, version = 22 ) => `${key}_${version}`,
	...
	gameLogFile : () => {
		if ( serveIPC.storeSet.get('game_log_auto') ) {
			return path.join(path.dirname(prefs.verGet('game_settings', prefs.ver())), 'log.txt')
		}
	...
	changeFilePath : (version, isGame = false) => {
		...
		const gameBinary = isWindows ? `FarmingSimulator20${version}.exe` : `FarmingSimulator20${version}`
		general.showFileDialog({
			defaultPath : isGame ?
				path.join(app.getPath('home'), gameBinary) :
				serveIPC.path.setFolder.replace(/FarmingSimulator20\d\d/, `FarmingSimulator20${version}`),
			extraFilter : isGame ?
				{ name : gameBinary, extensions : isWindows ? ['exe'] : [] } :
	...
	changeGameVersion : (newVersion) => {
		...
		gameSet.read()
		gameSet.setGameLog(false)
		serveIPC.windowLib.refreshL10n()
		serveIPC.refFunc.refreshClientModList()
	},
	clearVersion : ( version ) => {
		prefs.verSet('game_path', '', version)
		prefs.verSet('game_settings', '', version)
		prefs.verSet('game_enabled', false, version)
		prefs.verSet('game_args', '', version)
		return true
	},
	setNamed : (name, value) => {
		if ( name.startsWith('dev_mode') ) {
			...
			switch ( name ) {
				case 'game_version' :
					...
					serveIPC.windowLib.closeAllSubWin()
					serveIPC.windowLib.closeAllMultiWin()
					break
				case 'show_tooltips':
					serveIPC.storeSet.set(name, value)
					serveIPC.windowLib.refreshL10n()
...
const gameSet = {
	...
	_getXMLDoc : ( filename, ver ) => {
		...
		try {
			...
			gameSet.log.danger(`FS-${ver}`, 'Could not read XML file', filename, err)
			serveIPC.isPrefWrong = true
			return null
		}
	},
	...
	_write : async (filename, inputXML, opts) => {
	...
	_write_mp : (xml, opts) => {
		if ( opts.version === 25 || opts.version === 22 || opts.version === 19 ) {
			xml.gameSettings.joinGame ??= {}
	
			if ( opts.userName !== null && ( opts.version === 22 || opts.version === 25 ) ) { xml.gameSettings.onlinePresenceName = opts.userName }
	...
	_write_units : (xml, opts) => {
		if ( opts.version === 22 || opts.version === 25 ) {
	...
	change : async (CKey) => {
		if ( CKey === null ) { return gameSet.disable() }

		const currentVersion = serveIPC.storeNote.get(`${CKey}.notes_version`, 22)
		const filename       = prefs.verGet('game_settings', currentVersion)

		return gameSet._write(
			filename,
			gameSet._getXMLDoc(filename, currentVersion),
			{
				disable    : false,
				newFolder  : serveIPC.modCollect.mapCollectionToFolder(CKey),
				password   : serveIPC.storeNote.get(`${CKey}.notes_password`, null),
				serverName : serveIPC.storeNote.get(`${CKey}.notes_server`, null),
				unit_acre  : serveIPC.storeNote.get(`${CKey}.notes_unit_acre`, null),
				unit_miles : serveIPC.storeNote.get(`${CKey}.notes_unit_miles`, null),
				unit_money : serveIPC.storeNote.get(`${CKey}.notes_unit_money`, null),
				unit_temp  : serveIPC.storeNote.get(`${CKey}.notes_unit_temp`, null),
				userName   : serveIPC.storeNote.get(`${CKey}.notes_username`, null),
				version    : currentVersion,
			}
		)
	},
	disable : async () => {
		const currentVersion = prefs.ver()
		const filename       = prefs.verGet('game_settings', currentVersion)
	...
	read : ( newCollectKey = null ) => {
		...
		const filename = prefs.verGet('game_settings', currentVersion)
		const XMLDoc   = gameSet._getXMLDoc(filename, currentVersion)
		...
		try {
			...
			gameSet.log.danger(`FS-${currentVersion}`, 'Could not parse game settings', err)
			serveIPC.isPrefWrong = true
		}
	},
	...
	gameXML : (version = 22, setDevMode = null) => {
		const gameEnabledValue    = serveIPC.storeSet.get(`game_enabled_${version}`)
		...
		const XMLDoc = gameSet._getXMLDoc(gameXMLFile, version)
	...
	setGameLog : (newPath = false) => {
		if ( newPath !== false && fs.existsSync(newPath) ) {
	...
	streamGameLog : async () => {
		...
		if ( thisGameLog === null || !fs.existsSync(thisGameLog) ) {
			serveIPC.log.warning('game-log', 'Invalid game log', thisGameLog)
			return ''
		}

		serveIPC.log.debug('game-log', 'Reading game log', thisGameLog)

		return fsPromise.readFile(thisGameLog, { encoding : 'utf-8', flag : 'r'} ).catch((err) => {
...
const file = {
	...
	init : () => {
		if ( ! file.isInit ) {
			file.log    = serveIPC.log.group('file-operation')
			file.isInit = true
		}
	},
	process : async (operations) => {
		...
		for ( const operation of operations ) {
			operation.source_file = operation.source_rawPath ?? serveIPC.modCollect.mapColAndUUIDToFullPath(operation.source_collectKey, operation.source_modUUID)
			operation.dest_filename = path.basename(operation.source_file)
	
			if ( operation.type === 'copy'  || operation.type === 'favs' ) {
				promises.push(file._copy(operation).then((copyResult) => {
					results.push(...copyResult)
				}))
			} else if ( operation.type === 'move' ) {
				promises.push(file._move(operation).then((moveResult) => {
					results.push(...moveResult)
				}))
			} else if ( operation.type === 'delete' ) {
				promises.push(file._delete(operation).then((deleteResult) => {
					results.push(deleteResult)
				}))
			} else if ( operation.type === 'unzip' ) {
				promises.push(file._unzip(operation).then((unzipResult) => {
					results.push(unzipResult)
				}))
			}
		}
	...
	_copy : async (obj) => {
		...
		for ( const thisDest of obj.destinations ) {
			...
			promises.push(fsPromise.cp(obj.source_file, destFile, { recursive : true, force : true }).then(() => {
				...
				file.log.info('Copy failed', destFile, err.message)
				results.push(file._result_fail('copy', obj.source_file, destFile))
			}))
		}
		return Promise.allSettled(promises).then(() => results)
	},
	_delete : async (obj) => {
		file.log.info('Trashing', obj.source_file)

		return shell.trashItem(obj.source_file).then(() => {
			file.log.info('Delete succeed', obj.source_file)
			return file._result_ok('delete', obj.source_file)
		}).catch((err) => {
			file.log.info('Delete failed', obj.source_file, err.message)
			return file._result_fail('delete', obj.source_file)
		})
	},
	_move : async (obj) => {
		const results = []
		file.log.info('Move', obj.source_file)

		return file._copy(obj).then(async (copyResult) => {

			results.push(...copyResult)

			return fsPromise.rm(obj.source_file, { recursive : true, force : true }).then(() => {
				file.log.info('Removing original', obj.source_file)
				results.push(file._result_ok('delete', obj.source_file))
			}).catch((err) => {
				file.log.info('Delete failed', obj.source_file, err.message)
				results.push(file._result_fail('delete', obj.source_file))
			})

		}).catch(() => {
			file.log.info('One or more copies failed')
		}).finally(() => {
			return results
		})
	},
	_result : (status, type, source = null, destination = null) => ({
		dest   : destination,
		source : source,
		status : status,
		type   : type,
	}),
	_result_fail : (type, source = null, destination = null) => file._result(false, type, source, destination),
	_result_ok   : (type, source = null, destination = null) => file._result(true, type, source, destination),
	_unzip       : async(obj) => {
		const destFolder = serveIPC.modCollect.mapCollectionToFolder(obj.destinations[0])

		return new Promise((resolve, reject) => {
			const extract = unzip.Extract({ path : destFolder })

			extract.on('error', (err) => { reject(err) })

			fs.createReadStream(obj.source_file)
				.pipe(extract)
				.on('error', (err) => { reject(err) })
				.on('close', () => { resolve(true) })
		}).then(() => {
			file.log.info('Unzip succeed', obj.source_file, destFolder)
			return file._result_ok('unzip', obj.source_file)
		}).catch((err) => {
			file.log.danger('Import unzip failed', destFolder, err)
			return file._result_fail('unzip', obj.source_file, destFolder)
		})
	},
}
...
const saveManage = {
	...
	refresh : () => {
		saveManage.init()
		const thisSaveManage = new saveGameManager(prefs.verGet('game_settings', prefs.ver()))
	...
	restore : (fullPath, newSlot) => {
		saveManage.init()
		try {
			const newSlotFull = path.join(path.dirname(prefs.verGet('game_settings', prefs.ver())), `savegame${newSlot}`)
	...
	getImport : () => {
		...
		dialog.showOpenDialog(serveIPC.windowLib.win.save_manage, options).then((result) => {
			if ( !result.canceled ) {
				const results = JSON.parse(parseSaveGame(result.filePaths[0]))
				if ( results.errorList.length === 0 ) {
					serveIPC.windowLib.sendToValidWindow('save_manage', 'savemanage:import', result.filePaths[0])
				} else {
...
const gameLauncher = () => {
	...
	const currentVersion = prefs.ver()
	const gameArgs       = prefs.verGet('game_args', currentVersion).trim()
	const progPath       = prefs.verGet('game_path', currentVersion).trim()
...
const inputManage = {
	...
	init : () => {
		if ( ! inputManage.isInit ) {
			inputManage.log      = serveIPC.log.group('input-manager')
			inputManage.savePath = path.join(app.getPath('userData'), 'inputSets')
			inputManage.isInit   = true

			if ( ! fs.existsSync(inputManage.savePath) ) {
				fs.mkdirSync(inputManage.savePath)
			}
		}
	},
	...
	list : () => {
		inputManage.init()
		const fullList = []
		for ( const version of [19, 22, 25] ) {
			if ( prefs.verGet('game_enabled', version ) ) {
				const thisFile = path.join(path.dirname(prefs.verGet('game_settings', version)), 'inputBinding.xml')
				if ( fs.existsSync(thisFile) ) {
					inputManage.log.debug('Found active binding file', thisFile)
					fullList.push({
						date    : fs.statSync(thisFile).mtime,
		...
		if ( fs.existsSync(inputManage.savePath) ) {
			for ( const thisFile of fs.readdirSync(inputManage.savePath) ) {
				if ( path.extname(thisFile) === '.xml' ) {
					inputManage.log.debug('Found backup binding file', thisFile)
					fullList.push({
						date    : fs.statSync(path.join(inputManage.savePath, thisFile)).mtime,
						file    : thisFile,
						path    : path.join(inputManage.savePath, thisFile),
						type    : 'backup',
						version :  /_fs(\d\d)/.exec(thisFile)?.slice(1) | 0,
					})
				}
			}
		}
	...
	load : async (src) => {
		inputManage.log.info('Attempting to parse', src)
		const binds = new inputBinds(src)
		return binds.getInfo()
	},

	delete : async (src) => {
		inputManage.log.info('Attempting to delete', src)
		return fsPromise.rm(src).then(() => {
			return inputManage.list()
		}).catch((err) => {
			inputManage.log.warning('Delete failed!', err)
		})
	},

	copy : async (src, destShortName) => {
		inputManage.log.info('Attempting to copy', src, destShortName)
		return fsPromise.copyFile(src, path.join(inputManage.savePath, destShortName)).then(() => {
			return inputManage.list()
		}).catch((err) => {
			inputManage.log.warning('Copy failed!', err)
		})
	},

	restore : async (src, destVer) => {
		const thisFile = path.join(path.dirname(prefs.verGet('game_settings', destVer)), 'inputBinding.xml')
		inputManage.log.info('Attempting to restore', src, thisFile)
		return fsPromise.copyFile(src, thisFile).then(() => {
			return inputManage.list()
		}).catch((err) => {
			inputManage.log.warning('Restore failed!', err)
		})
	},
}
