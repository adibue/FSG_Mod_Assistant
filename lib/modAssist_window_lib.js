...
module.exports.windowLib = class {
	...
	winDefs = {
		...
		importjson : {
			callback        : (windowArgs) => { this.sendModList(windowArgs, 'importjson:data', 'importjson', false ) },
			handleURLinWin  : true,
		...
		mini : { // done
			...
			HTMLFile        : 'mini.html',
			refocusCallback : true,
			subWindowArgs   : { fixed : true,  noChrome : true, preload : 'sharedWindow', sizeable : false },
			winName         : 'mini',
		},
		notes : { //done
		...
		save : { // done
			callback        : (windowArgs) => { this.sendModList(windowArgs, 'save:collectName', 'save', false ) },
			extraCloseFunc  : () => {
		...
		save_manage : { //done
			callback        : (windowArgs) => { this.sendModList(windowArgs, 'savemanage:info', 'save_manage', false ) },
		...
		update : {
			callback        : () => { serveIPC.refFunc.refreshUpdateList() },
			handleURLinWin  : true,
			HTMLFile        : 'update.html',
			refocusCallback : true,
			subWindowArgs   : { preload : 'sharedWindow' },
			winName         : 'update',
		},
	 ...
	 */
	constructor() {
		this.loading  = new loadingWindow(this, 'main')
	 ...
	 */
	#getNativeIcon(icon, noDark = false) {
		const isDark   = noDark ? '' : nativeTheme.shouldUseDarkColors ? '' : 'blk-'
		const testIcon = funcLib.general.getPackPathRoot('icon', `${isDark}${icon}.ico`)

		return testIcon !== null ? testIcon : process.execPath
	}
	...
	populateContextIcons() {
		...
		this.contextIcons.dragDrop        = this.#getNativeImage('file-drag', true)
	}
	...
	trayContextMenu() {
		...
		const curCols  = serveIPC.modCollect.getCurVerCollections(funcLib.prefs.ver())
		const colStart = curCols
			.sort((a, b) => Intl.Collator().compare(b[3], a[3]))
			.sort((a, b) => Intl.Collator().compare(b[4], a[4]))
			.slice(0, 5)
			.map((x) => ({
				arguments   : `--start-game-with=${x[0]}`,
				description : this.l10n.syncStringLookup('launch_game_with'),
				iconIndex   : 0,
				iconPath    : this.#getNativeIcon('ui-icon-launch-col'),
		...
		app.setUserTasks([
			{
				...
				description : this.l10n.syncStringLookup('launch_game__title'),
				iconIndex   : 0,
				iconPath    : this.#getNativeIcon(`ui-icon-launch-${serveIPC.storeSet?.get?.('game_version', 22) || 22}`, true),
				program     : process.execPath,
				title       : this.l10n.syncStringLookup('launch_game'),
			},
			...colStart,
		])

		const curCollect = serveIPC.modCollect.getCurCollectSafe(serveIPC.gameSetOverride.folder)
		const launchExt  = curCollect === '' ? {} : { sublabel : curCollect }
		this.tray.setContextMenu(Menu.buildFromTemplate([
			funcLib.menu.iconL10n('app_name', () => { this.win.main.show() }, 'fsgMA'),
			funcLib.menu.sep,
			{
				click : () => { serveIPC.refFunc.gameLauncher() },
				icon  : this.#getNativeImage(`ui-icon-launch-${serveIPC.storeSet?.get?.('game_version', 22) || 22}`, true),
				label : this.l10n.syncStringLookup('launch_game'),
				...launchExt,
			},
			{
				icon    : serveIPC.windowLib.contextIcons.depend,
				label   : this.l10n.syncStringLookup('launch_game_with'),
				submenu : curCols.map((x) => ({
					click : () => { funcLib.gameSet.change(x[0]).then(() => { funcLib.gameLauncher() }) },
	 ...
	 */
	isValid(name) {
		return ( Object.hasOwn(this.win, name) && this.win[name] !== null && !this.win[name].isDestroyed() && !this.win[name].webContents.isLoading() )
	}
	...
	closeAllMultiWin () {
		for ( const thisWin in this.multiWin ) {
			if ( thisWin.isDestroyed() ) { continue }
			thisWin.destroy()
		}
	...
	destroyAndFocusMulti (focusTarget = 'main') {
		this.multiWin = this.multiWin.filter((x) => !x.isDestroyed())
		if ( focusTarget !== 'main' && this.isValid(focusTarget) && this.isVisible(focusTarget) ) {
			this.win[focusTarget].focus()
			return
		}
		if ( this.isValid('main') ) {
			if ( !this.isVisible('main') && this.isValid('mini')  && this.isVisible('mini') ) {
	 ...
	 */
	sendModList(extraArgs = {}, eventName = 'mods:list', toWindow = 'main', closeLoader = true) {
		...
		serveIPC.modCollect.toRenderer(extraArgs).then((modCollection) => {
			...
			if ( toWindow === 'main' && this.isVisible('update') ) {
				serveIPC.modCollect.toRenderer({
					activeCollection : serveIPC.gameSetOverride.index,
					modSites         : serveIPC.storeSites.store,
				}).then((updateModCollection) => {
					this.sendToValidWindow('update', eventName, updateModCollection)
				})
			}
	...
	themeUpdater() {
		for ( const thisWinKey in this.win ) {
			if ( this.isVisible(thisWinKey) ) {
				this.sendToWindow(thisWinKey, 'win:updateTheme', this.#themeCurrentColor)
		...
		this.multiWin = this.multiWin.filter((x) => !x.isDestroyed())
		for ( const displayWin of this.multiWin ) {
			displayWin.webContents.send('win:updateTheme')
			displayWin.setTitleBarOverlay({
				color       : this.#themeColors[this.#themeCurrentColor].background,
				symbolColor : this.#themeColors[this.#themeCurrentColor].font,
			})
		}
	...
	refreshL10n() {
		for ( const thisWinKey in this.win ) {
			this.sendToValidWindow(thisWinKey, 'i18n:refresh')
		}
		this.multiWin = this.multiWin.filter((x) => !x.isDestroyed())
		for ( const displayWin of this.multiWin) {
			displayWin.webContents.send('i18n:refresh')
		}
	}
	...
	fontUpdater() {
		for ( const thisWinKey in this.win ) {
			this.sendToValidWindow(thisWinKey, 'win:updateFontSize')
		}
		this.multiWin = this.multiWin.filter((x) => !x.isDestroyed())
		for ( const displayWin of this.multiWin) {
			displayWin.webContents.send('win:updateFontSize')
		}
	}
	...
	createNamedMulti(winName, windowArgs, extraCallback = null) {
		...
		thisWindow.on('closed', () => {
			...
			this.destroyAndFocusMulti(windowArgs.focusTarget)
		})
	 ...
	 */
	createMainWindow (openCallback, doSplashScreen = true) {

		this.win.main = this.createSubWindow('main', { noSelect : false, show : !doSplashScreen, preload : 'sharedWindow' })
		...
		this.win.main.webContents.on('render-process-gone', (_e, detail) => {
			serveIPC.log.warning('dev-note', 'Main window crashed, reloading', detail)
			this.hasCrashed = true
			this.win.main.webContents.reload()
			// this.createMainWindow(openCallback, false)
			
		})
	
		this.win.main.webContents.on('before-input-event', (event, input) => {
			if (input.control && input.code === 'KeyA') {
				this.win.main.webContents.send('select:all')
				event.preventDefault()
			}
			if (input.control && input.shift && input.code === 'KeyA' ) {
				this.win.main.webContents.send('select:none')
				event.preventDefault()
			}
			if (input.control && input.code === 'KeyI') {
				this.win.main.webContents.send('select:invert')
				event.preventDefault()
			}
			if ( input.alt && input.control && input.code === 'KeyD' ) {
				this.createNamedWindow('debug')
			...
			if ( input.code === 'Delete' ) {
				this.win.main.webContents.send('files:deleteTrigger')
				event.preventDefault()
			}
			if ( input.control && input.code === 'Space' ) {
				serveIPC.refFunc.gameLauncher()
...
class loadingWindow {
	...
	hide (time = 1250) {
		setTimeout(() => {
			if ( this.#win.isValid(this.#parent) ) {
				this.#win.win.main.setProgressBar(-1)
...
module.exports.defaultSettings = class {
	...
	get migrateBase () { return {
		'4.9.0' : (store) => {
			for ( const key of ['game_args', 'game_path', 'game_settings'] ) {
				if ( store.get(key, null ) === null ) { continue }
				store.set(`${key}_22`, store.get(key))
			}
			store.set('game_enabled_22', true)
			store.delete('game_path')
			store.delete('game_args')
			store.delete('game_settings')
			store.delete('multi_version')
			store.delete('wins.basegame')
			store.delete('wins.compare')
			store.delete('wins.detail')
		},
	}}
	...
	get defaults () { return {
		...
		game_version      : { type : 'number',  default : 25, enum : [25, 22, 19, 17, 15, 13]},
		...
		game_enabled_25   : { type : 'boolean', default : true},
		game_path_25      : { type : 'string',  default : serveIPC.path.game || '' },
		game_settings_25  : { type : 'string',  default : serveIPC.path.setFile || '' },

		game_args_22      : { type : 'string',  default : '' },
		game_enabled_22   : { type : 'boolean', default : false},
		game_path_22      : { type : 'string',  default : '' },
		game_settings_22  : { type : 'string',  default : '' },
		...
		wins              : { type : 'object', default : {}, properties : {
			basegame19    : this.#winDef(1000, 650),
			basegame22    : this.#winDef(1000, 650),
			basegame25    : this.#winDef(1000, 650),
			change        : this.#winDef(650, 350),
			compare19     : this.#winDef(1000, 650),
			compare22     : this.#winDef(1000, 650),
			compare25     : this.#winDef(1000, 650),
			debug         : this.#winDef(1000, 500),
			detail19      : this.#winDef(800, 500),
			detail22      : this.#winDef(800, 500),
			detail25      : this.#winDef(800, 500),
			devtools      : this.#winDef(1000, 700),
			find          : this.#winDef(800, 600),
			folder        : this.#winDef(800, 500),
			gamelog       : this.#winDef(1000, 500),
			history       : this.#winDef(1000, 600),
			import        : this.#winDef(750, 500),
			importjson    : this.#winDef(900, 700),
			input         : this.#winDef(1000, 650),
			...
			update        : this.#winDef(900, 600),
			vault         : this.#winDef(1000, 650),
