...
const appIconFile = process.platform === 'win32' ? 'icon.ico' : process.platform === 'darwin' ? 'icon.icns' : 'icon.png'
const sourceRoot = path.join(__dirname, '..')
const packagedRoot = path.join(process.resourcesPath, 'app.asar')
const runtimeRoot = fs.existsSync(path.join(sourceRoot, 'renderer')) ? sourceRoot : packagedRoot
...
module.exports.windowLib = class {
	...
	loading   = null
	#path     = {
		icon    : path.join(runtimeRoot, 'build', appIconFile),
		preload : path.join(runtimeRoot, 'renderer', 'preload'),
		render  : path.join(runtimeRoot, 'renderer'),
	}
	...
	#getNativeImage(icon, noDark = false) {
		const basePath = path.join(runtimeRoot, 'renderer', 'img', 'context-icons')
	...
	trayContextMenu() {
		...
		const colStart = curCols
			.sort((a, b) => new Intl.Collator().compare(b[3], a[3]))
			.sort((a, b) => new Intl.Collator().compare(b[4], a[4]))
			.slice(0, 5)
			.map((x) => ({
